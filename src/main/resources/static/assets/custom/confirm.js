/**
 * Bootstrap Confirm Delete Author: Tom Kaczocha <tom@rawphp.org> Licensed under
 * the MIT license
 */
...
(function($, window, document, undefined) {
		...
		this.onDelete = function(event) {
			...
			if (null !== plugin.settings.callback) {
				if ($.isFunction(plugin.settings.callback)) {
					deleteBtn.attr('data-dismiss', 'modal').off(
							'.bs-confirm-delete').on('click.bs-confirm-delete',
							{
								originalObject : $(this)
							}, plugin.settings.callback);
					hasCallback = true;
				} else {
					console.log(plugin.settings.callback
			...
			if (null !== plugin.settings.delete_callback) {
				if ($.isFunction(plugin.settings.delete_callback)) {
					deleteBtn.attr('data-dismiss', 'modal').off(
							...
							{
								originalObject : $(this)
							}, plugin.settings.delete_callback);
					hasCallback = true;
				} else {
					console.log(plugin.settings.delete_callback
							+ ' is not a valid callback');
				}
			}
			...
			if (null !== plugin.settings.cancel_callback) {
				cancelBtn.off('.bs-confirm-delete').on(
						'click.bs-confirm-delete', {
							originalObject : $(this)
						}, plugin.settings.cancel_callback);
			}
		};
	};
	...
	$.fn.bootstrap_confirm_delete = function(options) {
		return this.each(function() {
			...
			element.off('.bs-confirm-delete').on('click.bs-confirm-delete',
					plugin.onDelete);
...
$('.delete-application-confirm').bootstrap_confirm_delete({
...
$('.delete-project-confirm').bootstrap_confirm_delete({
...
$('.scale-down-confirm').bootstrap_confirm_delete({
	heading : 'Confirm Scale Down',
	message : 'Are you sure you want to scale down to 0 replicas? This will stop all pods for the deployment.',
